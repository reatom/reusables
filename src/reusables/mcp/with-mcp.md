...
### Atoms — automatic registration on first init

Atoms represent **readable state snapshots**. The tool is registered automatically the first time the atom is initialised.
...
// Tool registers itself on first read:
goodsAtom()
```

### Custom params mapper

Use `params` to remap tool input to multi-argument actions:
...
| `params`       | `(input, client, target) => args` | forward `input` as first arg                 | Maps tool input to action arguments.                  |
